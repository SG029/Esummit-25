...
function App() {
  ...
  const [isMobileView, setIsMobileView] = useState(window.innerWidth < 768); // Track if the view is mobile
  ...
  useEffect(() => {
    const handleResize = () => {
      setIsMobileView(window.innerWidth < 768); // Set mobile view if width is less than 768px
    };
        ...
        <AnimatePresence>
          {!isLoading && (
            <motion.div
              ...
              transition={{ duration: 1, ease: "easeOut" }}
            >
              {/* Vectors (only show if not in mobile view) */}
              {!isMobileView && (
                <>
                  {/* Bluevector */}
                  {!isBlueVectorFinal && (
